...
test('默认id为客户端IP', async () => {
  ...
  const app = new WebApp({
    mount: mdchain.web.mount(rateLimit({ store })),
  });
...
test('携带额外报头', async () => {
  const app = new WebApp({
    mount: mdchain.web.mount(rateLimit({ maxRequest: 20 })),
  });
...
test('超过速率后抛出429状态码', async () => {
  const app = new WebApp({
    mount: mdchain.web.mount(rateLimit({ maxRequest: 2 })),
  });
...
test('retry-after格式为秒', async () => {
  const app = new WebApp({
    mount: mdchain.web.mount(rateLimit({ maxRequest: 1, duration: 3600_000 })),
  });
  await supertest(app.listen()).get('/');
  await supertest(app.listen())
    .get('/')
...
test('可以取消headers', async () => {
  const app = new WebApp({
    mount: mdchain.web.mount(rateLimit({ headers: false })),
  });
  await supertest(app.listen())
    .get('/')
    .expect((res) => {
...
test('定制重置周期', async () => {
  const app = new WebApp({
    mount: mdchain.web.mount(rateLimit({ duration: 1500, maxRequest: 2 })),
  });
...
test('定制报错信息', async () => {
  const app = new WebApp({
    mount: mdchain.web.mount(rateLimit({ errorMessage: 'Wait a minute', maxRequest: 1 })),
  });
  await supertest(app.listen()).get('/').expect(404);
