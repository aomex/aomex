...
test('ipv4格式', async () => {
  ...
  await expect(validator['validate']('::1', '')).resolves.toStrictEqual(
    magistrate.fail('必须是IPv4地址', '', []),
  );
});
...
test('ipv6格式', async () => {
  ...
  await expect(validator['validate']('0.0.0.0', '')).resolves.toStrictEqual(
    magistrate.fail('必须是IPv6地址', '', []),
  );
});
...
test('一些不合法的ip', async () => {
  ...
  for (const data of [
    ...
    await expect(validator['validate'](data, '')).resolves.toStrictEqual(
      magistrate.fail('必须是IP[v4,v6]地址', '', []),
    );
  }
});
...
test('获取文档', () => {
  ...
  `);
  expect(new IpValidator(['v6']).match(/\.com$/)['toDocument']())
