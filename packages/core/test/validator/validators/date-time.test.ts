...
describe('时间戳', () => {
  test('默认不解析时间戳', async () => {
    ...
    expect(result).toStrictEqual(magistrate.fail('必须是时间类型', '', []));
  });
...
describe('范围', () => {
  test('最小时间（包含）', async () => {
    const validator = new DateTimeValidator().min(
      () => new Date(1711257_956_000),
    );
    await expect(
      validator['validate'](new Date(1711257_956_001)),
    ).resolves.toStrictEqual(magistrate.ok(new Date(1711257_956_001)));
    await expect(
      validator['validate'](new Date(1711257_956_000)),
    ).resolves.toStrictEqual(magistrate.ok(new Date(1711257_956_000)));
    await expect(
      validator['validate'](new Date(1711257_955_999), ''),
    ).resolves.toStrictEqual(magistrate.fail('不在指定时间内', '', []));
  });
  ...
  test('最大时间（包含）', async () => {
    const validator = new DateTimeValidator().max(
      () => new Date(1711257_956_000),
    );
    await expect(
      validator['validate'](new Date(1711257_955_999)),
    ).resolves.toStrictEqual(magistrate.ok(new Date(1711257_955_999)));
    await expect(
      validator['validate'](new Date(1711257_956_000)),
    ).resolves.toStrictEqual(magistrate.ok(new Date(1711257_956_000)));
    await expect(
...
test('获取文档', () => {
  ...
  expect(
    new DateTimeValidator().default(new Date(1711257_956_000))['toDocument'](),
