...
test('属性验证失败报错', async () => {
  ...
  expect(result).toStrictEqual(
    magistrate.fail('必须是对象类型', 'data', ['path_1']),
  );
});
...
describe('解析字符串', () => {
  ...
  test('非法字符串', async () => {
    ...
    expect(result1).toStrictEqual(magistrate.fail('必须是对象类型', '', []));
    const result2 = await validator['validate'](JSON.stringify('abc'), '');
    expect(result2).toStrictEqual(magistrate.fail('必须是对象类型', '', []));
  });
  ...
  test('严格模式，默认不从字符串恢复', async () => {
    ...
    const data = { key: 'value' };
    const result = await validator['validate'](JSON.stringify(data), '');
    expect(result).toStrictEqual(magistrate.fail('必须是对象类型', '', []));
  });
