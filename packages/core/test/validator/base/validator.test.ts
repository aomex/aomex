...
describe('transform()', () => {
  test('正常执行指定的transform函数', async () => {
    const validator = new MockValidator()['transform'](
      (data) => data + '-hello',
    );
...
describe('optional()', async () => {
  ...
  test('允许为空', async () => {
    for (const value of empty) {
      await expect(
        validator['optional']()['validate'](value, ''),
      ).resolves.toStrictEqual(magistrate.ok(undefined));
    }
  });
});
...
describe('补充文档', () => {
  ...
  test('replace模式', () => {
    expect(
      Validator.toDocument(validator['docs']({ title: 'test3' }, 'replace')),
    ).toMatchInlineSnapshot(`
      {
        "required": true,
        "schema": {
