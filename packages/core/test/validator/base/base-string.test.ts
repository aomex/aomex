...
describe('长度', () => {
  ...
  test('长度区间', async () => {
    ...
    await expect(validator['validate']('01', '')).resolves.toStrictEqual(
      magistrate.fail('字符串长度不合法', '', []),
    );
    await expect(validator['validate']('01234')).resolves.toStrictEqual(
      magistrate.ok('01234'),
...
test('匹配格式', async () => {
  ...
  await expect(validator['validate']('abd', '')).resolves.toStrictEqual(
    magistrate.fail('未匹配指定格式', '', []),
  );
