...
import { container, middleware } from '../../src';
...
test('挂载中间件', () => {
  const c = container.mixin.mount(md1).mount(md2);
  expect(c['middlewareList']).toStrictEqual([md1, md2]);
});

test('挂载空值', () => {
  const c = container.mixin.mount(md1).mount(null);
  expect(c['middlewareList']).toStrictEqual([md1]);
});

test('挂载容器会被拍平为中间件', () => {
  const c1 = container.mixin.mount(md1);
  const c2 = container.mixin.mount(md2).mount(c1);
...
test('创建分割点', () => {
  const c1 = container.mixin.mount(md1).mount(md2).mount(md2).mount(md1);
  const p1 = c1['createPoint']();
  expect(p1).toBeTypeOf('string');
});

test('按分割点切出新的容器', () => {
  const c1 = container.mixin.mount(md1).mount(md2);
  const p1 = c1['createPoint']();
