...
import { container, flattenMiddlewareToken, middleware } from '../../src';
...
test('链条容器', () => {
  ...
  const myChain = container.mixin.mount(md1).mount(md2);
  ...
  expect(flattenMiddlewareToken([null, md1, myChain])).toStrictEqual([
    md1,
    md1,
    md2,
  ]);
