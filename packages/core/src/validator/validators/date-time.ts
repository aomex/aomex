...
export class DateTimeValidator<T = Date> extends Validator<T> {
  ...
  protected validateValue(
    ...
    if (date === false || date.toString() === 'Invalid Date') {
      return magistrate.fail('必须是时间类型', key, superKeys);
    }

    if (!this.compare(date)) {
      return magistrate.fail('不在指定时间内', key, superKeys);
    }
  ...
  protected compare(date: Date) {
    ...
    if (
      min !== undefined &&
      (minInclusive ? timestamp < +min() : timestamp <= +min())
    ) {
      return false;
    }
    if (
      max !== undefined &&
      (maxInclusive ? timestamp > +max() : timestamp >= +max())
    ) {
      return false;
    }
    return true;
  ...
  protected override toDocument(): OpenAPI.SchemaObject {
    const defaultValue: Date | undefined = this.getDefaultValue(
